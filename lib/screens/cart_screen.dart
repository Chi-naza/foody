    ...
    return Scaffold(
      body: Padding(
        ...
        child: SingleChildScrollView(          
          child: Obx(() {
            // if cart is empty
              return productController.cartProductsList.isEmpty? Container(
                  ...
                  SizedBox(
                    ...
                    child: ListView.separated(
                      itemBuilder: (BuildContext context, int index) {
                        var cartItem = productController.cartProductsList[index];
                        var total = cartItem.quantity * double.parse(cartItem.product.discountPrice);
                        return InkWell(
                          onTap: () {
                            Get.to(CategoryDetailScreen(product: cartItem.product));
                          },
                          child: CartItemWidget(
                            productName: cartItem.product.name,
                            productIndex: index, 
                            quantity: cartItem.quantity, 
                            totalPrice: productController.formatToMoney(total.toString()), 
                            image: FoodyAPI.BASE_URL + cartItem.product.image,                      
                          ),
                        );
                      },
                  ...
                  Container(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        showChoiceDialog(
                          title: 'Make an Order', 
                          message: 'You want to continue with your order?', 
                          okOnPressed: (){
                            Get.back();
                            // Show bottom sheet
            ...
            children: [
              ...
              FoodyMainButton(
                ...
                onTapped: (){
                  ...
                  if(fName.isEmpty || lName.isEmpty || email.isEmpty || address.isEmpty || city.isEmpty || postalCode.isEmpty || state.isEmpty || phone.isEmpty){
                    authController.showSweetToast(message: "Wrong or empty fields. Kindly fill it properly and try agaain", isSuccess: false);
                  }else {
                    List<CartModel> cartList = [];

                    for(var co in productController.cartProductsList){
                      // instance of cartModel
                      var cartModel = CartModel(productId: co.product.id.toString(), quantity: co.quantity, price: co.price);
                    ...
                    if(cartList.isEmpty){
                      authController.showSweetToast(message: "Error: No item in the cart", isSuccess: false);
                    }else{
                      ...
                      if(kDebugMode) print("ORDER OBJECT CREATED: $cOderModel");

                      // calling our method
                      productController.makeAnOrder(cOderModel);
                      }
                  }
                },

...
class PaymentController extends GetxController {
  ...
  Future<void> createPaymentReference(
      {required String orderID,
      required String cardNumber,
      required String CVV,
      required String expMonth,
      required String expYear,
      required String atmPin}) async {
    ...
    if (response.statusCode == 200) {
      ...
      Get.to(PaymentVerificationScreen(
          orderID: orderID, flwRef: response.body["flwRef"].toString()));
    } else {
      authController.showSweetToast(
          message: "Payment failed", isSuccess: false);
    }
  }

  // This method starts the payment process for the user making an order
  Future<void> validateUsersPayment(
      {required String orderID,
    ...
    Response response = await HelperAPIMethods.postData(
      uri:
          FoodyAPI.BASE_URL + FoodyAPI.validatePaymentEndpoint(orderID, flwRef),
      withHeader: true,
      body: validationBody,
    );
